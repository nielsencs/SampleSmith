...
- **Visible DS controls** — when effects are enabled, SampleSmith also writes a simple Decent Sampler `<ui>` tab with visible knobs bound to the correct effect positions where the binding parameter is known. `K` checkboxes decide which main controls appear as DS knobs; the generated display starts with Tone, then a boxed Reverb section with Amount, Room, and Damp controls, then Delay and Chorus, followed by the remaining enabled effect knobs. Knobs include Decent Sampler `defaultValue` settings, and each SampleSmith effect row has a small defaults button for restoring sensible documented/example defaults.
