...
- **Decent Sampler root-note convention** — SampleSmith keeps note names literal in the UI, but exports Decent Sampler `rootNote` values one octave higher so a SampleSmith C4 lands on the C4 position in Carl's Decent Sampler/Reaper workflow. This replaces the old user-facing root-offset knob with one fixed export convention.
...
The **Effective exported sample mapping** table shows which WAV will play on which MIDI keys, including the Decent Sampler root note number used for each displayed SampleSmith note.
