...
To diagnose octave-label / MIDI-number confusion between SampleSmith, Decent Sampler, Reaper, and a controller, generate the octave diagnostic pack:
...
- **Root notes are MIDI numbers** — Decent Sampler `rootNote`, `loNote`, and `hiNote` are MIDI note numbers. SampleSmith exports the stored MIDI root literally and keeps the old user-facing root-offset knob removed. Use `tools/make_octave_diagnostic_pack.py` if a host/controller labels octaves differently.
