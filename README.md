...
- **Root notes export unchanged** — a sample mapped as C4 exports with `rootNote` C4. Earlier root-offset correction has been removed so note names stay literal.
...
The **Effective exported sample mapping** table shows which WAV will play on which MIDI keys, including root notes, so octave-label differences between Python/Reaper/Decent Sampler do not hide what is actually mapped.
