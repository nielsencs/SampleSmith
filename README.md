...
Set the instrument name and output folder, choose **Pitched** or **Unpitched / Pads**, then record. By default, single recordings start immediately; enable **Confirm before recording** if you want the old ready-check prompt. Pitched recordings can also play a reference tone first, controlled by **Play reference before pitched recording**. After capture, SampleSmith opens a review box so you can play the raw/trimmed recording, adjust trim dB and trim again, keep, redo, or skip. Existing WAV files still ask before being replaced. Each recording writes/updates both the `.dspreset` and the `.samplesmith.json` project file automatically; the generate/save buttons are there for manual regeneration.
